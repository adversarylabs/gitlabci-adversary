#!/usr/bin/env node
import { createRequire as __adversaryCreateRequire } from 'node:module'; const require = __adversaryCreateRequire(import.meta.url);
...
var MAX_FILES = 5e3;
var MAX_SOURCE_BYTES = 75e4;
var execute = promisify(execFile);
var RESERVED_TOP_LEVEL_KEYS = /* @__PURE__ */ new Set([
  "after_script",
  "before_script",
  "cache",
  "default",
...
var RELEASE_JOB = /(?:^|[-_.])(deploy(?:ment)?|publish(?:ing)?|release)(?:$|[-_.])/i;
async function analyzeRepository(ctx) {
  const allPaths = await walk2(ctx.repoPath);
  const scoped = await ctx.loadInScopeSources({
    include: (path) => !path.split("/").some((segment) => SKIPPED.has(segment)) && isYamlPath(path),
    limit: MAX_FILES
  });
  ...
  const reviewedByPath = new Map(
    sources.filter(
      (file) => configurationPaths.has(file.path) || spec.files.some((glob) => matchesGlob(file.path, glob))
...
function detectInterruptibleReleases(rule, sources, configuration, conventionalFiles) {
  ...
  const configured = configuration === void 0 ? [] : configuration.sources.filter((file) => file.inScope || configuration.root.inScope).flatMap((file) => detectInterruptibleRelease(rule, file, configuration.root));
  const standalone = sources.filter((file) => !configuredPaths.has(file.path) && conventionalFiles.some((glob) => matchesGlob(file.path, glob))).flatMap((file) => detectInterruptibleRelease(rule, file, file));
...
function detectInterruptibleRelease(rule, file, root) {
  ...
  const rootDefaultInterruptible = rootDefaultBlock === void 0 ? void 0 : booleanProperty(rootDefaultBlock, "interruptible");
  const fileDefaultInterruptible = fileDefaultBlock === void 0 ? void 0 : booleanProperty(fileDefaultBlock, "interruptible");
  const defaultInterruptible = rootDefaultInterruptible ?? fileDefaultInterruptible;
  const defaultBlock = rootDefaultInterruptible === void 0 ? fileDefaultBlock : rootDefaultBlock;
  const defaultFile = rootDefaultInterruptible === void 0 ? file.path : root.path;
  return blocks.flatMap((block) => {
    if (RESERVED_TOP_LEVEL_KEYS.has(block.key) || block.key.startsWith(".")) return [];
    const stage = scalarProperty(block, "stage");
    if (!RELEASE_JOB.test(block.key) && (stage === void 0 || !RELEASE_JOB.test(stage))) return [];
    if (isTagOnly(block)) return [];
    const jobInterruptible = booleanProperty(block, "interruptible");
    if (jobInterruptible === false) return [];
    const inherited = jobInterruptible === void 0 && defaultInterruptible === true;
    ...
    return [{
      ...
      data: {
        ...
        defaultLine: inherited ? defaultBlock?.line : void 0,
        defaultFile: inherited ? defaultFile : void 0,
...
async function discoverGitlabConfiguration(repoPath, allPaths, inScopeSources) {
  ...
  if (rootPath === void 0) return void 0;
  const byPath = new Map(inScopeSources.map((file) => [file.path, file]));
  const ensureSource = async (path) => {
    const existing = byPath.get(path);
    if (existing !== void 0) return existing;
    try {
      const content = await readFile4(join3(repoPath, path));
      if (content.byteLength > MAX_SOURCE_BYTES || content.includes(0)) return void 0;
      const source = {
        path,
  ...
  const discovered = /* @__PURE__ */ new Map([[root.path, root]]);
  const queue = [root];
  while (queue.length > 0) {
    const current = queue.shift();
    if (current === void 0) break;
    const patterns = localIncludePatterns(current.source);
    const includedPaths = allPaths.filter(
      (path) => isYamlPath(path) && patterns.some((pattern) => matchesGlob(path, pattern))
    );
...
function localIncludePatterns(source) {
  ...
  if (include === void 0) return uniqueLocalPatterns(patterns);
  const directIndentation = directChildIndentation(include);
  if (directIndentation === void 0) return uniqueLocalPatterns(patterns);
  let localListIndentation;
  for (const line of include.lines.slice(1)) {
    if (line.trim() === "" || line.trimStart().startsWith("#")) continue;
    const indentation = line.search(/\S/);
    if (localListIndentation !== void 0 && indentation <= localListIndentation) localListIndentation = void 0;
    ...
    if (localListIndentation !== void 0 && indentation > localListIndentation) {
      const item = /^\s*-\s*(.*?)\s*(?:#.*)?$/.exec(line)?.[1];
      if (item !== void 0) patterns.push(...parseIncludeValues(item));
      continue;
    }
    const shorthand = /^\s*-\s*(.*?)\s*(?:#.*)?$/.exec(line)?.[1];
    if (shorthand !== void 0 && indentation === directIndentation) patterns.push(...parseIncludeValues(shorthand));
  }
  return uniqueLocalPatterns(patterns);
}
function parseIncludeValues(value) {
  const trimmed = value.trim();
  const values = trimmed.startsWith("[") && trimmed.endsWith("]") ? trimmed.slice(1, -1).split(",") : [trimmed];
  return values.map((entry) => normalizeLocalPattern(entry)).filter((entry) => entry !== void 0);
}
function normalizeLocalPattern(value) {
  ...
  if (normalized === "" || normalized.includes("$") || normalized.includes(":") || /(?:^|\/)\.\.(?:\/|$)/.test(normalized) || !/\.ya?ml$/i.test(normalized)) return void 0;
  return normalized;
}
...
function hasAutoCancelDisabled(blocks) {
  ...
  if (autoCancel === void 0) return false;
  const indentation = directChildIndentation({ ...workflow, lines: autoCancel });
  if (indentation === void 0) return false;
...
function booleanProperty(block, property) {
  ...
  if (indentation === void 0) return void 0;
...
function directChildIndentation(block) {
  const indentations = block.lines.slice(1).filter((line) => line.trim() !== "" && !line.trimStart().startsWith("#")).map((line) => line.search(/\S/)).filter((indentation) => indentation > 0);
  return indentations.length === 0 ? void 0 : Math.min(...indentations);
}
...
// src/index.ts
function createApp() {
  const app = new Adversary({ name: "gitlab-ci", version: "0.0.11", review: { maximumFindings: 8 } });
  registerRules(app);
